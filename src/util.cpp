...
const char * const BITCOIN_CONF_FILENAME = "hexus.conf";
const char * const BITCOIN_PID_FILENAME = "hexusd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "hexus" is a composite category enabling all Hexus-related debug output
                if(ptrCategory->count(std::string("hexus"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Hexus
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Hexus
    // Mac: ~/Library/Application Support/Hexus
    // Unix: ~/.hexus
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Hexus";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Hexus";
#else
    // Unix
    return pathRet / ".hexus";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty hexus.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override hexus.conf
