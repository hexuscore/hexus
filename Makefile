...
pkgdatadir = $(datadir)/hexuscore
pkgincludedir = $(includedir)/hexuscore
pkglibdir = $(libdir)/hexuscore
pkglibexecdir = $(libexecdir)/hexuscore
...
CONFIG_HEADER = $(top_builddir)/src/config/hexus-config.h
CONFIG_CLEAN_FILES = libhexusconsensus.pc share/setup.nsi \
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	$(srcdir)/libhexusconsensus.pc.in \
	...
	$(top_srcdir)/src/config/hexus-config.h.in \
...
ACLOCAL = ${SHELL} /tmp/Hexus/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /tmp/Hexus/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /tmp/Hexus/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /tmp/Hexus/build-aux/missing automake-1.15
AWK = gawk
BDB_CFLAGS = 
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = hexus-cli
BITCOIN_DAEMON_NAME = hexusd
BITCOIN_GUI_NAME = hexus-qt
BITCOIN_TX_NAME = hexus-tx
...
COPYRIGHT_HOLDERS_FINAL = The Hexus Core developers
COPYRIGHT_HOLDERS_SUBSTITUTION = Hexus Core
COPYRIGHT_YEAR = 2019
CPP = gcc -E
...
MAINT = 
MAKEINFO = ${SHELL} /tmp/Hexus/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = hexuscore
PACKAGE_BUGREPORT = https://github.com/hexuspay/hexus/issues
PACKAGE_NAME = Hexus Core
PACKAGE_STRING = Hexus Core 0.12.3
PACKAGE_TARNAME = hexuscore
PACKAGE_URL = https://hexus.org/
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /tmp/Hexus
abs_srcdir = /tmp/Hexus
abs_top_builddir = /tmp/Hexus
abs_top_srcdir = /tmp/Hexus
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /tmp/Hexus/build-aux/install-sh
...
pkgconfig_DATA = libhexusconsensus.pc
...
DIST_CONTRIB = $(top_srcdir)/contrib/hexus-cli.bash-completion \
	       $(top_srcdir)/contrib/hexus-tx.bash-completion \
	       $(top_srcdir)/contrib/hexusd.bash-completion \
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info \
  leveldb_baseline.info test_hexus_filtered.info total_coverage.info \
  baseline_filtered.info rpc_test.info rpc_test_filtered.info \
  leveldb_baseline_filtered.info test_hexus_coverage.info test_hexus.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/Hexus-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/hexus-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/hexus-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/hexus-config.h
$(top_srcdir)/src/config/hexus-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/hexus-config.h src/config/stamp-h1
libhexusconsensus.pc: $(top_builddir)/config.status $(srcdir)/libhexusconsensus.pc.in
...
$(OSX_APP)/Contents/MacOS/Hexus-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Hexus-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Hexus-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_hexus.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_hexus -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_hexus_filtered.info: test_hexus.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#rpc_test.info: test_hexus_filtered.info
...
#test_hexus_coverage.info: baseline_filtered_combined.info test_hexus_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_hexus_filtered.info -o $@

#total_coverage.info: baseline_filtered_combined.info test_hexus_filtered.info rpc_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_hexus_filtered.info -a rpc_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_hexus.coverage/.dirstamp:  test_hexus_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_hexus.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf coverage_percent.txt test_hexus.coverage/ total.coverage/ qa/tmp/ cache/ $(OSX_APP)
